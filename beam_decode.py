...
class Seq2SQL(nn.Module):
    ...
    def forward(self, all_inp_var, all_name_len, all_len, q_embedding, q_len, type_embedding ,type_len, type_new_embedding , type_new_len, gt_seq = None, reinforce=False, gt_sel=None):
        ...
        e_new_type, h_type = run_lstm(self.encoder, type_new_embedding ,type_new_len)
        ...
        e_all_expand = torch.cat((e_all_expand, e_new_type), 2)
        ...
        if gt_seq is not None:
            ...
            _, ans_tok_var = cur_cond_score.view(B, max_x_len).topk(2)
    ...
    def loss(self, score, gt_seq ,all_tokens, all_type, col ,cell, all_cell, k):
        ...
        for b in range(len(gt_seq)):
            if self.gpu:
                ...
                cond_truth_var = Variable(torch.from_numpy(np.array(gt_seq[b][1:])))
            cond_pred_score = cond_score[b, :len(gt_seq[b])-1]
            cond_pred = torch.nn.functional.softmax(cond_pred_score)
            _,choice = cond_pred_score.topk(1)
            choice = choice.squeeze(1)

            greedy_sample = []
            for score in cond_pred_score.data.cpu().numpy():
                tok = np.argmax(score)
                val = all_tokens[b][tok]
                if val == '<END>':
                    break
                greedy_sample.append(val)

            random_samples = []
            samples = cond_pred.multinomial(5).permute(1,0)
            #samples_list = samples.cpu().numpy().tolist()
            for s in range(len(samples)):
                random_sample = []
                for ss in samples[s]:
                    vall = all_tokens[b][ss]
                    if vall == '<END>':
                        break
                    random_sample.append(all_tokens[b][ss])
                random_samples.append(random_sample)

            for i in range(len(random_samples)):
                reward = reward + (grammar(random_samples[i], col[b], cell[b], all_tokens[b],all_type[b],all_cell[b])-grammar(greedy_sample, col[b], cell[b], all_tokens[b],all_type[b],all_cell[b])) * self.CE(cond_pred_score,samples[i])/5
            reward = Variable(reward)
            ce = self.CE(cond_pred_score, cond_truth_var)
            loss += ((k * reward + (1-k) * ce) / len(gt_seq))

        return loss

    def step(self, all_inp_var, all_name_len, all_len, q_embedding, q_len, type_embedding ,type_len, type_new_embedding , type_new_len):
    ...
    def sample(self, all_inp_var, all_name_len, all_len, q_embedding, q_len, type_embedding ,type_len, type_new_embedding , type_new_len, all_seq):
        """
        ...
        output = self.step(all_inp_var, all_name_len, all_len, q_embedding, q_len, type_embedding ,type_len, type_new_embedding , type_new_len)
    ...
    def MCsample(self, all_inp_var, all_name_len, all_len, q_embedding, q_len, type_embedding ,type_len, type_new_embedding , type_new_len, all_tok, data):
        x = data
        s, index = x.topk(1)
